...
async def predict_price(request: PredictionRequest):
    ...
    try:
        ...
        price_data_year = data.get("price_data_year")
        price_data_month = data.get("price_data_month")
        price_data_date = (
            f"{price_data_year}-{int(price_data_month):02d}"
            if price_data_year and price_data_month
            else None
        )
        environmental_data = {
